    ...
    public static partial class CodeGen
    {
        ...
        public static void GenerateConstructor(Type t)
        {
            ...
            t.ProcessMembers(GenTaskFlags.DefaultConstructor, false, info =>
            {
                ...
                if (info.type.IsConfig() && info.insideConfigStorage == false) return;
                ...
                if (!info.isReadOnly && info.type.IsLivableCustomType())
                {
                    if (info.justData)
                    {
                        // if its just data then new instant of this should not be created for now... tbd
                        return;
                    }
                ...
                if (info.type.IsLoadableConfig() && t.IsLivableCustomType()) return;
            ...
            if (t.HasAttribute<GenModelRootSetup>())
            {
                ...
                constructor.content($"{SetupChildrenIdFuncName}();");
            }
        }
        ...
        static string NewInstExpr(Type t, bool pooled, object constructorArg = null)
        {
            ...
            {
                return $"new {t.RealName(true)}({constructorArg ?? ""})";
            }
            else
            {
