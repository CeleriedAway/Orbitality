...
namespace ZergRush.CodeGen
{
    public static partial class CodeGen
    {
        ...
        public static string ConfigStorageDictionary = "ConfigDictionary";
        ...
        public static void GenCollectConfigs(Type type, string funcPrefix)
        {
            TraverseGenCustomType(new TraversStrategy
            {
                ...
                elemProcess = (sink, info) =>
                {
                    ...
                    {
                        sink.content($"_collection.Add({info.access}.UId(), {info.access});");
                    }
                    if (info.type.IsCollectableConfigType())
                    {
                        RequestGen(info.type, GenTaskFlags.CollectConfigs);
                        sink.content($"{info.access}.{collectConfigFuncName}(_collection);");
                    }
                },
                memberPredicate = info => info.type.IsList() || info.type.IsDictionary() || (info.type.ReadGenFlags() & GenTaskFlags.UIDGen) != 0,
                funcArgs = "ConfigStorageDictionary _collection"
            }, type, funcPrefix);
        }
    }
}