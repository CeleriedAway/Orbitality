    ...
    public class VirtualVerticalLayoutGroup : LayoutGroup, IPointerClickHandler
    {
        ...
        public object SelectedItem
        {
            ...
            set
            {
                ...
                // Ensure that the new selectedTypeName item is present in the item list
                if (value != null && newSelectedIndex < 0)
                {
                    throw new InvalidOperationException("Cannot select item not present in layout");
                }

                // Invalidate old selectedTypeName item row
                ...
                // Invalidate the newly selectedTypeName item
                if (_selectedItem != null)
                {
                    InvalidateItem(_selectedIndex);
                }
        ...
        public override void SetLayoutHorizontal()
        {
            ...
            // Hide non-enabled rows to one side. More efficient than enabling/disabling them
        ...
        private void PopulateRow(int index, Row row)
        {
            ...
            {
                // If there is a selectedTypeName row stylesheet, and this is the selectedTypeName row, use that one
