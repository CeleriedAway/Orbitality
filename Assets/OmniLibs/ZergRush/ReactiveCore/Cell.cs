...
namespace ZergRush.ReactiveCore
{
    ...
    public class Cell<T> : ICellRW<T>, IConnectable
    {
        ...
        public IEventReader changed => updates;
    ...
    public static class CollectionReactiveApi
    {
        ...
        public static IDisposable Bind<T>(this IReactiveCollection<T> list, Action<List<T>> action)
        {
            action(list.current);
            return list.update.Subscribe(_ => action(list.current));
        }
        
        public static IDisposable BindCollection<T>(this IReactiveCollection<T> list, Action<ReactiveCollectionEvent<T>> action)
        {
            action(new ReactiveCollectionEvent<T>{type = ReactiveCollectionEventType.Reset, newData = list.current});
        ...
        public static class CellReactiveApi
    {
        ...
        public static IEventReader<T> UpdateStream<T>(this ICell<T> cell)
        {
            if (cell is Cell<T>) return (cell as Cell<T>).updates;
            return new AnonymousEventReader<T>(cell.ListenUpdates);
        }
...
//        }
        public static void Subscribe<T>(this IEventReader<T> e, IConnectionSink connectionSink, Action<T> action)
        {
        ...
        public static IEventReader<T2> FlatMap<T, T2>(this ICell<T> cell, Func<T, IEventReader<T2>> map)
        {
            return cell.Map(v => map(v)).Join();
        }
        
        public static IEventReader<T2> FlatMapWithDefaultOnNull<T, T2>(this ICell<T> cell, Func<T, IEventReader<T2>> map)
            where T : class
        {
            return cell.FlatMap(v => v != null ? map(v) : AbandonedReader<T2>.value);
        }
        ...
        public static IEventReader<T> Join<T>(this ICell<IEventReader<T>> cell)
        {
            return new AnonymousEventReader<T>(reaction =>
            {
                var group = new DoubleDisposable();
                Action<IEventReader<T>> func = (IEventReader<T> innerStream) =>
                {
                    if (group.disposed) return;
        ...
        public static ICellRW<T> ReflectionFieldToRW<T>(this object obj, string fieldName)
        {
            ...
            if (f == null) {Debug.LogError($"field {fieldName} is not found in obj {obj}"); return null;}
        ...
        public static IEventReader<(T, T)> BufferPreviousValue<T>(this ICell<T> cell)
        {
            // Implicit lambda boxing used as a prev val storage here
            var prevVal = cell.value;
            return new AnonymousEventReader<(T, T)>(action =>
            {
                return cell.ListenUpdates(v =>
                {
                    action((v, prevVal));
        ...
        // it that is so it waits gate to be true, to update its value from initial cell
        public static ICell<T> Gate<T>(this ICell<T> cell, ICell<bool> gate, Action<IDisposable> connectionSink)
        {
...
//            }, curr);
        ...
        public static ICell<T> Hold<T>(this IEventReader<T> eventReader, T initial, Action<IDisposable> connectionSink)
        {
            var cell = new Cell<T>(initial);
            connectionSink(eventReader.Subscribe(val => cell.value = val));
            return cell;
        }
        ...
        public static IEventReader WhenOnce<T>(this ICell<T> cell, Func<T, bool> filter)
        {
        ...
        public static IEventReader WhenOnce(this ICell<bool> cell)
        {
        ...
        public static IEventReader When<T>(this ICell<T> cell, Func<T, bool> filter)
        {
            return new AnonymousEventReader(reaction =>
            {
        ...
        public static IEventReader WhenEqualsOnce<T>(this ICell<T> cell, T value)
        {
        ...
        public static IEventReader WhenUpdatedToSatisfy<T>(this ICell<T> cell, Func<T, bool> filter)
        {
            return new AnonymousEventReader((Action reaction) =>
            {
                return cell.ListenUpdates(val =>
        ...
        public static IEventReader When(this ICell<bool> cell)
        {
            return cell.When(i => i);
        }

        public static IEventReader WhenMoreOrEqual(this ICell<float> cell, float value)
        {
            return cell.When(v => v >= value);
        }
        
        public static IEventReader WhenMoreOrEqual(this ICell<int> cell, int value)
        {
            return cell.When(v => v >= value);
        }
        
        public static IEventReader WhenUpdatedToTrue(this ICell<bool> cell)
        {
