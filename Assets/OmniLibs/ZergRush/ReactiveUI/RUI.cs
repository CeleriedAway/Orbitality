...
namespace ZergRush.ReactiveUI
{
    ...
    public static partial class Rui
    {
            ...
            where TView : ReusableView
        {
            ...
            {
                var count = components.collection.current.Count;
                if (count == 0) return;
                ...
                else
                {
                    ...
                    viewStorage.EnsureLoadedInterval(firstVisible, lastVisible, collection.current);
                }
            };
                ...
                ) where TView : ReusableView
        {
            ...
            if (pool == null)
            {
                ...
                {
                    pool = new DistinctivePool<TView, TData>(parent, prefabSelector, options.Has(PresentOptions.UseLoadedViews));
                }
                else
                {
                    ...
                    {
                        FillPoolWithChildrenViews(pool, parent, actualPrefab.GetType());
                    }
                }
            }
            ...
            if (updater != null)
            {
                ...
                {
                    showCopy(item, view);
                    view.addConnection = updater(item).Subscribe(() => showCopy(item, view));
                };
        ...
        ) where TView : ReusableView where TViewGroup : ReusableView
        {
            ...
            groupComponents.addConnection = collection.BindCollection(ev =>
            {
                if (ev.type == ReactiveCollectionEventType.Reset)
                {
                    ...
                    foreach (var groupView in grouped)
                    {
                        groupView.layout.count = groupView.collection.current.Count;
        ...
        public static SimplePresentComponents<TView, T> Present<T, TView>(
            ...
            Func<T, IEventReader> updater = null,
            ...
            ) where TView : ReusableView
        {
            ...
            viewStorage.ReloadAll(coll.current);
